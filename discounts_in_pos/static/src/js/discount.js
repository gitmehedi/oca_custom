openerp.discouts_in_pos = function (instance) {
        ...
        set_value: function (val) {
            ...
            if (order.getSelectedLine()) {
                ...
                // for the new button 'DiscFix'
                    ...
                    console.log(order)
                    console.log(order.getSelectedLine())
                    order.getSelectedLine().set_discount_fixed(val);
                }
                else if (mode === 'discount_total') {
                    order.getSelectedLine().set_discount_total(val);
                    order.set_discount_total_order(val);
                }
                else if (mode === 'discount_percent') {
                    order.getSelectedLine().set_discount_percent(val);
...
//    ============================updates total discount(fixed and percentage) on order ==========================================
        update_summary: function () {
            ...
            this.el.querySele1ctor('.summary .total .subentry .value').textContent = this.format_currency(taxes);
            ...
            if (quantity === 'remove') {
                ...
                if (unit) {
                    if (unit.rounding) {
                        ...
                        // this.quantityStr = this.format_currency(round_di(this.quantity, decimals), {
                        //     type: 'float',
                        //     digits: [69, decimals]
                        // });
        ...
        push_and_invoice_order: function (order) {
            ...
            this.flush_mutex.exec(function () {
                ...
                transfer.pipe(function (order_server_id) {
                    ...
                    self.chrome.do_action('discounts_in_pos.pos_invoice_report', {
                        additional_context: {
